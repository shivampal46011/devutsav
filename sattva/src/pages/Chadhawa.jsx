...
const Chadhawa = () => {
    const [chadhawas, setChadhawas] = useState([]);
    ...
    useEffect(() => {
        const fetchMarketData = async () => {
            ...
            try {
                ...
                setChadhawas(Array.isArray(cList) ? cList : []);
            } catch (error) {
    ...
    return (
        ...
        <section className="w-full px-6 mb-8 bg-gradient-to-b from-primary/10 to-transparent pt-12 pb-8 border-b border-primary/10">
          <h1 className="font-headline text-4xl md:text-5xl font-black tracking-tight text-on-surface leading-tight mb-4 text-left">
            Sacred <span className="text-primary italic font-serif tracking-normal">Chadhawa</span>
          </h1>
          ...
          <div className="flex flex-col gap-4 text-base font-bold text-on-surface-variant max-w-md w-full">
            <div className="flex items-center justify-start gap-3">
              <span className="material-symbols-outlined text-primary text-2xl flex-shrink-0" style={{ fontVariationSettings: "'FILL' 1" }}>local_florist</span> 
              <span className="text-left leading-tight">Authentic Temple Offerings</span>
            </div>
            <div className="flex items-center justify-start gap-3">
              ...
              <span className="text-left leading-tight">Offered in Your Name-Gotra</span>
            </div>
            <div className="flex items-center justify-start gap-3">
              <span className="material-symbols-outlined text-primary text-2xl flex-shrink-0" style={{ fontVariationSettings: "'FILL' 1" }}>redeem</span> 
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                            {chadhawas.length === 0 && (
                                <div className="col-span-full py-16 text-center text-on-surface-variant bg-surface-container rounded-3xl border border-dashed border-outline-variant/50">
                                    ...
                                    <p className="font-bold">No Chadhawas found at this moment.</p>
                                </div>
                            )}

                            {chadhawas.map(item => renderCard(item, 'Chadhawa Offering'))}
                        </div>
                    )}
                </section>
            </main>
