    ...
    return (
        <>
            <main className="px-4 md:px-0 max-w-5xl mx-auto relative pt-8 pb-32 min-h-screen bg-[#FDF9F5]">
                <section className="px-4 md:px-6 mb-8 text-center bg-gradient-to-b from-primary/10 to-transparent pt-12 pb-8 rounded-3xl mx-2 shadow-inner border border-primary/10">
                    <h1 className="font-headline text-4xl md:text-5xl font-black tracking-tight text-on-surface leading-tight mb-4">
                        DevPunya <br/><span className="text-primary italic font-serif tracking-normal">Marketplace</span>
                    </h1>
                    ...
                    <div className="flex justify-center">
                        <div className="bg-surface-container-highest p-1.5 rounded-full inline-flex shadow-inner">
                            {/* 'Chadhawas' tab hidden for now — code kept below */}
                            {['Pujas'].map(tab => (
                                <button
                                    key={tab}
                                    onClick={() => setActiveTab(tab)}
                                    className={`px-8 py-2.5 rounded-full text-sm font-bold tracking-wide transition-all duration-300 ${activeTab === tab ? 'bg-white text-primary shadow-sm' : 'text-on-surface-variant hover:text-on-surface'}`}
                                >
                                    {tab}
                                </button>
                            ))}
                ...
                <section className="px-4 md:px-6">
                    {isLoading ? (
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                            {activeTab === 'Pujas' && pujas.length === 0 && (
                            ...
                            {activeTab === 'Chadhawas' && chadhawas.length === 0 && (
                                <div className="col-span-full py-16 text-center text-on-surface-variant bg-surface-container rounded-3xl border border-dashed border-outline-variant/50">
                                    <span className="material-symbols-outlined text-5xl mb-3 opacity-50">search_off</span>
                            ...
                            {activeTab === 'Pujas' && pujas.map(item => renderCard(item, 'Live Puja'))}
                            {activeTab === 'Chadhawas' && chadhawas.map(item => renderCard(item, 'Chadhawa Offering'))}
                        </div>
                    )}
                </section>
            </main>
