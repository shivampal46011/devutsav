...
export default component$(() => {
  ...
  const form = useStore({
    ...
    dob: '', tob: '', pob: '', pob_lat: null as number | null, pob_lon: null as number | null,
    ...
    <main class="px-4 md:px-0 max-w-5xl mx-auto relative pt-12 pb-24">
      ...
      {step.value === 1 && (
        <section class="px-6 max-w-md mx-auto">
          <div class="bg-surface-container rounded-[2rem] p-8 shadow-sm border border-outline-variant/10">
            <div class="flex items-center justify-between mb-8">
              <span class="font-label text-xs uppercase tracking-widest text-primary font-bold">Step 1 of 2</span>
              <span class="text-sm font-bold text-on-surface-variant">Personal Info</span>
            </div>
            ...
            <div class="space-y-5">
              <div>
                <label class="block text-xs font-bold uppercase tracking-widest text-on-surface-variant mb-2">Full Name *</label>
                <input value={form.name} onInput$={(e) => (form.name = (e.target as HTMLInputElement).value)} class="w-full bg-surface-container-low border border-outline-variant/20 rounded-xl px-4 py-3 text-on-surface focus:outline-none focus:ring-2 focus:ring-primary/50" placeholder="e.g. Rahul Sharma" />
              </div>
              <div class="flex gap-3">
                <div class="w-[30%]">
                  <label class="block text-xs font-bold uppercase tracking-widest text-on-surface-variant mb-2">ISD</label>
                  <input value={form.isd_code} onInput$={(e) => (form.isd_code = (e.target as HTMLInputElement).value)} class="w-full bg-surface-container-low border border-outline-variant/20 rounded-xl px-4 py-3 text-on-surface text-center focus:outline-none focus:ring-2 focus:ring-primary/50" />
                </div>
                <div class="flex-1">
                  <label class="block text-xs font-bold uppercase tracking-widest text-on-surface-variant mb-2">Phone *</label>
                  <input type="tel" value={form.phone} onInput$={(e) => (form.phone = (e.target as HTMLInputElement).value)} class="w-full bg-surface-container-low border border-outline-variant/20 rounded-xl px-4 py-3 text-on-surface focus:outline-none focus:ring-2 focus:ring-primary/50" placeholder="10 Digits" />
                </div>
              </div>
              <div>
                <label class="block text-xs font-bold uppercase tracking-widest text-on-surface-variant mb-2">Email (Optional)</label>
                <input type="email" value={form.email} onInput$={(e) => (form.email = (e.target as HTMLInputElement).value)} class="w-full bg-surface-container-low border border-outline-variant/20 rounded-xl px-4 py-3 text-on-surface focus:outline-none focus:ring-2 focus:ring-primary/50" placeholder="rahul@example.com" />
              </div>
            </div>
            <button
              onClick$={async () => {
              ...
              class="w-full mt-8 py-4 bg-gradient-to-r from-primary to-primary-container text-white font-bold rounded-xl shadow-lg active:scale-95 transition-all text-sm tracking-widest uppercase"
            >
      ...
      {step.value === 2 && (
        <section class="px-6 max-w-md mx-auto">
          <div class="bg-surface-container rounded-[2rem] p-8 shadow-sm border border-outline-variant/10">
            <div class="flex items-center justify-between mb-8">
              <button onClick$={() => (step.value = 1)} class="text-on-surface-variant hover:text-primary transition-colors flex items-center text-sm font-bold uppercase tracking-widest">
                <span class="material-symbols-outlined text-sm mr-1">arrow_back</span> Back
              </button>
              <span class="text-sm font-bold text-on-surface-variant">Birth Details</span>
            </div>
            {validationError.value && (
              <div class="bg-error/10 text-error text-sm font-bold px-4 py-3 rounded-xl mb-6 flex items-center gap-2">
                <span class="material-symbols-outlined text-sm">error</span>{validationError.value}
            ...
            <div class="space-y-5">
              <div>
                <label class="block text-xs font-bold uppercase tracking-widest text-on-surface-variant mb-2">Date of Birth *</label>
                <input type="date" value={form.dob} onInput$={(e) => (form.dob = (e.target as HTMLInputElement).value)} class="w-full bg-surface-container-low border border-outline-variant/20 rounded-xl px-4 py-3 text-on-surface focus:outline-none focus:ring-2 focus:ring-primary/50" />
              </div>
              <div>
                <div class="flex items-center justify-between mb-2">
                  <label class="block text-xs font-bold uppercase tracking-widest text-on-surface-variant">Time of Birth (24Hr)</label>
                  <label class="flex items-center gap-2 text-xs font-bold text-on-surface-variant cursor-pointer">
                    <input type="checkbox" checked={tobUnknown.value} onChange$={() => (tobUnknown.value = !tobUnknown.value)} class="w-4 h-4 rounded text-primary cursor-pointer" />
                    <span>I don't know</span>
                  </label>
                </div>
                <input disabled={tobUnknown.value} type="time" value={form.tob} onInput$={(e) => (form.tob = (e.target as HTMLInputElement).value)} class="w-full bg-surface-container-low border border-outline-variant/20 rounded-xl px-4 py-3 text-on-surface focus:outline-none focus:ring-2 focus:ring-primary/50 disabled:opacity-50 disabled:cursor-not-allowed" />
              </div>
              <div class="relative">
                <label class="block text-xs font-bold uppercase tracking-widest text-on-surface-variant mb-2">Place of Birth *</label>
                <div class="relative">
                  <input
                    ...
                    class="w-full bg-surface-container-low border border-outline-variant/20 rounded-xl px-4 py-3 text-on-surface focus:outline-none focus:ring-2 focus:ring-primary/50"
                  />
                  {isSearchingPob.value && <div class="absolute right-4 top-3.5 w-4 h-4 border-2 border-primary/30 border-t-primary rounded-full animate-spin" />}
                </div>
                <span class="text-[10px] text-on-surface-variant/60 mt-2 block pl-1">Select matched city from dropdown!</span>
                {pobSuggestions.value.length > 0 && (
                  <div class="absolute top-[80px] w-full bg-white border border-outline-variant/30 rounded-xl shadow-2xl z-20 overflow-hidden max-h-56 overflow-y-auto">
                    {pobSuggestions.value.map((place: any, idx: number) => (
                      <div key={idx} class="px-4 py-3 hover:bg-surface-variant border-b border-outline-variant/10 cursor-pointer text-sm font-medium transition-colors text-on-surface" onClick$={() => selectPob(place.description, place.place_id)}>
                        {place.description}
                      </div>
                    ))}
                  </div>
            ...
            <button
              onClick$={async () => {
                ...
                if (!form.dob) { validationError.value = 'Date of Birth is required.'; return; }
                if (!tobUnknown.value && !form.tob) { validationError.value = "Time of Birth required or check 'I don't know'."; return; }
                ...
                try {
                  ...
                  const [kundaliRes] = await Promise.all([
                    fetch(`${api}/api/kundali/analyze-dosha`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ user_id: uId, user_session_id: sessionId.value, dob: form.dob, tob: tobUnknown.value ? null : form.tob, tob_unknown: tobUnknown.value, pob: form.pob, pob_lat: form.pob_lat, pob_lon: form.pob_lon }) }),
              ...
              class="w-full mt-8 py-4 bg-gradient-to-r from-secondary to-tertiary text-white font-bold rounded-xl shadow-lg shadow-secondary/20 active:scale-95 transition-all text-sm tracking-widest uppercase flex justify-center items-center gap-2"
            >
