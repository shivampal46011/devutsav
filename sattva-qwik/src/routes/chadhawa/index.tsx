...
export default component$(() => {
  ...
  useVisibleTask$(async () => {
    try {
      ...
      const chadhawaRes = await fetch(`${apiBase}/api/market/chadhawas`);
      const chadhawaData = await chadhawaRes.json();
      const cList = chadhawaData?.results?.data || chadhawaData?.results || chadhawaData || [];
      chadhawas.value = Array.isArray(cList) ? cList : [];
    } catch (err) {
      ...
      <main class="px-4 md:px-0 max-w-5xl mx-auto relative pt-8 pb-32 min-h-screen bg-[#FDF9F5]">
        <section class="w-full px-6 mb-8 bg-gradient-to-b from-primary/10 to-transparent pt-12 pb-8 border-b border-primary/10">
          <h1 class="font-headline text-4xl md:text-5xl font-black tracking-tight text-on-surface leading-tight mb-4 text-left">
            Sacred <span class="text-primary italic font-serif tracking-normal">Chadhawa</span>
          </h1>
          <p class="text-on-surface-variant text-lg font-medium max-w-lg mb-8 text-left border-l-4 border-primary/30 pl-4">
            Offer blessed Chadhawa and Prasad to the deities across prominent temples in India.
          </p>
          <div class="flex flex-col gap-4 text-base font-bold text-on-surface-variant max-w-md w-full">
            <div class="flex items-center justify-start gap-3">
              <span class="material-symbols-outlined text-primary text-2xl flex-shrink-0" style="font-variation-settings: 'FILL' 1">local_florist</span> 
              <span class="text-left leading-tight">Authentic Temple Offerings</span>
            </div>
            <div class="flex items-center justify-start gap-3">
              ...
              <span class="text-left leading-tight">Offered in Your Name-Gotra</span>
            </div>
            <div class="flex items-center justify-start gap-3">
              <span class="material-symbols-outlined text-primary text-2xl flex-shrink-0" style="font-variation-settings: 'FILL' 1">redeem</span> 
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {chadhawas.value.length === 0 && (
                <div class="col-span-full py-16 text-center text-on-surface-variant bg-surface-container rounded-3xl border border-dashed border-outline-variant/50">
                  ...
                  <p class="font-bold">No Chadhawas found at this moment.</p>
                </div>
              )}
              {chadhawas.value.map((item: any) => {
                const id = item.id;
                const title = item.name || 'Sacred Offering';
                const image = item.image_url || item.images?.[0] || item.png_default_image || 'https://images.unsplash.com/photo-1604068545831-7e8717a03009?auto=format&fit=crop&q=80&w=800';
                let price = item.price || null;
                if (!price && item.offeringDetails && Array.isArray(item.offeringDetails) && item.offeringDetails.length > 0) {
                  price = Math.min(...item.offeringDetails.map((p: any) => parseInt(p.price) || Infinity));
                  if (price === Infinity) price = null;
                }
                if (!price && item.packages && Array.isArray(item.packages) && item.packages.length > 0) {
                ...
                return (
                  <div
                    ...
                    onClick$={() => window.open(`https://devpunya.com/chadhaava/v2/${id}?utm_content=devutsav&affiliate_id=34180`, '_blank')}
                  >
